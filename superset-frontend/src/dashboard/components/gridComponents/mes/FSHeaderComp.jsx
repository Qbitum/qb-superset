...
const GridHeader = styled.div`
  ${({ theme, editMode }) => css`
    ...
    &.grid-row--empty {
      min-height: ${theme.gridUnit * 15}px;
    }
  `}
`;

const emptyHeaderContentStyles = theme => css`
  position: absolute;
  width: 100%;
  height: 100%;
  display: flex;
...
class FSHeaderComp extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      isFocused: false,
        ...
        {({ dragSourceRef }) => (
          ...
          >
            ...
            >
              ...
              {rowItems.length === 0 && (
                <div css={emptyHeaderContentStyles}>
                  {t('Empty Main Header')}
                </div>
              )}
              {rowItems.length > 0 &&
...
FSHeaderComp.propTypes = propTypes;

export default FSHeaderComp;
