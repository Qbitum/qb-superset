...
import { Top3ListValue, PluginChartMESTop3ListStylesProps } from './types';
...
class MESTop3List extends React.PureComponent<PluginChartMESTop3ListStylesProps> {
  static defaultProps = {
    className: '',
  ...
  render() {
    ...
    // prepare view model
    const viewModel: Top3ListValue[] = [];

    if (values) {
      values.forEach((val: { data: any }, i: any) => {
        viewModel.push({
          title: subtitle[i],
          data: val.data || 0,
        });
...
export default styled(MESTop3List)`
  ${({ theme }) => `
